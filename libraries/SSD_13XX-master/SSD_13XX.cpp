...
#else //All the rest
	SSD_13XX::SSD_13XX(const uint8_t cspin,const uint8_t dcpin,const uint8_t rstpin, const uint8_t mosi, const uint8_t sclk, const uint8_t spi_bus): SPI(spi_bus)
	{
		_cs   = cspin;
		_dc   = dcpin;
		_rst  = rstpin;
...
//initialize Vars
	...
	*csport |= cspinmask;//hi
	...
	pinMode(_dc, OUTPUT);
	pinMode(_cs, OUTPUT);
	if (!avoidSPIinit) SPI.begin(_sclk, 12,_mosi, _cs);
	#if !defined(SPI_HAS_TRANSACTION)
		if (!avoidSPIinit){
			SPI.setClockDivider(4);
			SPI.setBitOrder(MSBFIRST);
